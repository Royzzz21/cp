...
class AdminPressMgr extends Controller{
...
//			return view('admin.member.list')->with('users', $users);
			...
			$list_info = DB::select("select * from press order by seq ASC");

//			print_r($list_info);
			return view('admin/press/list', compact('list_info','req_path1','req_path2','req_path3','page','bbs_id','_CFG','is_admin'));
...
//                echo $new_order_val;
                ...
                foreach($new_seq as $k=>$v){
                    DB::update("update press set seq=? where idx=?",[$v,$k]);
                }
		...
		if(Auth::check()) {
			if ($bbs_cmd == 'add') {
				...
				if (intval($idx) == 0) {
					...
					$edit_mode = 1;
					$rows = DB::select("select * from press where idx=? limit 1", [$idx]);

					$rows=$rows[0];

				}

				return view('admin/press/add', compact('idx','list_info', 'req_path1', 'req_path2', 'req_path3', 'page', 'bbs_id', '_CFG', 'is_admin', 'rows', 'edit_mode'));
			}
		}
		else{
			$error_msg='로그인이 필요 합니다.';
		...
		if($bbs_cmd=='add_ok') {
            ...
            if(strlen($name_en)==0 || strlen($name_ko)==0 || strlen($link_url)==0 || strlen($news_title)==0){
            ...
            if(strlen($idx)==0 || intval($idx)==0){
                ...
                $insert_cnt=DB::insert($q,[$name_en,$name_ko,$posting_date,$link_url,$news_title,$memo]);
                ...
                if($insert_cnt){
                    ...
                    if($file_is_uploaded) {
                        ...
                        Input::file('photo1')->move('photo/press_photo', $new_name);
                        DB::update("update press set photo_ext=? where idx=?",[$extension,$insert_id]);
            ...
            // 기존 수정
                ...
                DB::update($q,[$name_en,$name_ko,$posting_date,$link_url,$news_title,$memo,$idx]);

                if($file_is_uploaded) {
                    $new_name = $idx . '.' . $extension;
                    Input::file('photo1')->move('photo/press_photo', $new_name);
                    DB::update("update press set photo_ext=? where idx=?",[$extension,$idx]);
                }
            }
			...
			// 이동할 페이지 주소
            $move_url=$req_path2;
            return view('admin/press/add_ok', compact('move_url'));
		...
		if($bbs_cmd=='delete') {
			...
			$result = DB::delete("delete from press where idx=?", [$idx]);
