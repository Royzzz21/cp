...
class AdminMediaMgr extends Controller{
...
//			return view('admin.member.list')->with('users', $users);
			...
			$list_info = DB::select("select * from media order by seq ASC");

//			print_r($list_info);
			return view('admin/media/list', compact('list_info','req_path1','req_path2','req_path3','page','bbs_id','_CFG','is_admin'));
...
//                echo $new_order_val;
                ...
                foreach($new_seq as $k=>$v){
                    DB::update("update media set seq=? where idx=?",[$v,$k]);
                }
		...
		if(Auth::check()) {
			if ($bbs_cmd == 'add') {
				...
				if (intval($idx) == 0) {
					...
					$edit_mode = 1;
					$rows = DB::select("select * from media where idx=? limit 1", [$idx]);

					$rows=$rows[0];

				}

				return view('admin/media/add', compact('idx','list_info', 'req_path1', 'req_path2', 'req_path3', 'page', 'bbs_id', '_CFG', 'is_admin', 'rows', 'edit_mode'));
			}
		}
		else{
			$error_msg='로그인이 필요 합니다.';
            ...
            if(strlen($idx)==0 || intval($idx)==0){
                ...
                if($insert_cnt){
                    ...
                    if($file_is_uploaded) {
                        ...
                        Input::file('photo1')->move('photo/media_photo', $new_name);
                        DB::update("update media set photo_ext=? where idx=?",[$extension,$insert_id]);
                    }
...
//                    $insert_id = DB::getPdo()->lastInsertId();
                    ...
                    if($file_is_uploaded) {
                        ...
                        Input::file('photo1')->move('photo/media_photo', $new_name);
                        DB::update("update media set photo_ext=? where idx=?",[$extension,$idx]);
                    }
//                }
            }
			...
			// 이동할 페이지 주소
            $move_url=$req_path2;
            return view('admin/media/add_ok', compact('move_url'));
		...
		if($bbs_cmd=='delete') {
			...
			$result = DB::delete("delete from media where idx=?", [$idx]);
