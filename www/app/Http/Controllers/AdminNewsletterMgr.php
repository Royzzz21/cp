...
class AdminnewsletterMgr extends Controller{
...
//			return view('admin.member.list')->with('users', $users);
			...
			$list_info = DB::select("select * from newsletter order by seq ASC");

//			print_r($list_info);
			return view('admin/newsletter/list', compact('list_info','req_path1','req_path2','req_path3','page','bbs_id','_CFG','is_admin'));
...
//                echo $new_order_val;
                ...
                foreach($new_seq as $k=>$v){
                    DB::update("update newsletter set seq=? where idx=?",[$v,$k]);
                }
		...
		if(Auth::check()) {
			if ($bbs_cmd == 'add') {
				...
				if (intval($idx) == 0) {
					...
					$edit_mode = 1;
					$rows = DB::select("select * from newsletter where idx=? limit 1", [$idx]);

					$rows=$rows[0];

				}

				return view('admin/newsletter/add', compact('idx','list_info', 'req_path1', 'req_path2', 'req_path3', 'page', 'bbs_id', '_CFG', 'is_admin', 'rows', 'edit_mode'));
			}
		}
		else{
			$error_msg='로그인이 필요 합니다.';
...
//            echo $idx;exit;
            ...
            if(strlen($idx)==0 || intval($idx)==0){
                ...
                $insert_cnt=DB::insert($q,[$name_en,$name_ko,$memo]);
                ...
                if($insert_cnt){
                    ...
                    if($file_is_uploaded) {
                        ...
                        Input::file('photo1')->move('photo/newsletter_photo', $new_name);
                        DB::update("update newsletter set photo_ext=? where idx=?",[$extension,$insert_id]);
            ...
            // 기존 수정
                ...
                DB::update($q,[$name_en,$name_ko,$memo,$idx]);
...
//                    $insert_id = DB::getPdo()->lastInsertId();
                    ...
                    if($file_is_uploaded) {
                        ...
                        Input::file('photo1')->move('photo/newsletter_photo', $new_name);
                        DB::update("update newsletter set photo_ext=? where idx=?",[$extension,$idx]);
                    }
//                }
            }
			...
			// 이동할 페이지 주소
            $move_url=$req_path2;
            return view('admin/newsletter/add_ok', compact('move_url'));
		...
		if($bbs_cmd=='delete') {
			...
			$result = DB::delete("delete from newsletter where idx=?", [$idx]);
