...
class NoticeCon extends BbsCon {
	...
	public function __construct()	{
        $this->set_bbsId('notice');
        $this->set_write_title('Post Notice');
        $this->set_guest_write(false);
        $this->set_guest_allowlist(true);
		$this->set_guest_allowview(true);

        // 페이지 정보를 설정합니다.
        glo()->page_obj->set_pageId('notice');
        glo()->page_obj->set_pageName('notice');
...
//        $this->notice_run('list',$request);
	...
	// 처리
	public function notice_run($bbs_cmd,Request $request){
        $this->auth();
        return $this->run($bbs_cmd,$request);
	}

    // admin > 처리
    public function notice_admin($bbs_cmd,Request $request){
