{{--@php ($layout_file = "layouts.admin.app")--}}
<?php
if(glo()->is_admin_page){
    $layout_file='layouts.admin.app';
}
else {
    $layout_file='layouts.coin.sub';

}
?>
@extends($layout_file)
@section('content')
    <style>
        #bbs_table th {
            font-size:12px;
        }
        #bbs_table th,td {
            padding:10px;
        }
    </style>

    <section class="content">
        <div class="clearfix">
            <div>

            </div>
            <div>

                <div id='content_bbs' style="">


                    <script>
                        function form_chk() {
                            var frm = document.search_form;
                            if (frm.search_box.value == "") {
                                alert('검색어를 입력하세요');
                                return false;
                            }
                            else {
                                return true;
                            }
                        }
                    </script>


                    <style>
                        .col_header {
                            height: 24px;
                            color: black;
                            font-weight: bold;
                            font-size: 11px;
                            font-family: dotum;
                            background-color: white;
                            border-bottom: 1px solid #dddddd;
                        }
                    </style>

                    <script>
                        var total_faq_cnt=<?=count($list_info)?>;

                        function close_all_faq(){
                            for(var i=0;i<total_faq_cnt;i++){
                                $('#faq_tr_'+i).hide();

                            }
                        }
                        function faq(num){
                            close_all_faq();
                            $('#faq_tr_'+num).show();

                        }
                    </script>
                    <?php

                    //                    $no = ($page*$list_row_limit);

                    //페이지 변수가 없을땐 페이지는 1이다
                    //					if(!$page){
                    //						$page=1;
                    //					}

                    $s=($page-1)*$list_row_limit; // 시작번호
                    $tot=count($list_info);
                    $no=$tot-$s; // 가상 번호

                    ?>

                    <table id='bbs_table' class="table-bordered" border=1 cellspacing='0' cellpadding='0' width=100% style="padding:0 !important;;border:1px solid #d5d5d5;">
                        <tr>
                            <th style="width:50px;padding:10px;">no.</th>
                            <th style="width:100px;text-align:center;">created</th>
                            <th style="width:100px;text-align:center;">remaining time</th>

                            <th style="width:50px;text-align:center;">Project Id</th>
                            <th style="width:;text-align:center;">Task Name</th>
                            <th style="width:;text-align:center;">Idx</th>
                            <th style="width:;text-align:center;">Parent Idx</th>

                            <th style="width:;text-align:center;">Progress</th>
                            <th style="width:;text-align:center;">Type</th>
                            <th style="width:;text-align:center;">Priority</th>
                            <th style="width:;text-align:center;">Created by</th>
                            <th style="width:5%;text-align:center;">Assignee</th>
                            <th style=";text-align:center;">Started at</th>
                            <th style=";text-align:center;">Status</th>
                            <th style=";text-align:center;"></th>

                        </tr>
                        <?php
                        foreach($list_info as $row){

                            $reg_time = $row->bbs_regtt;

                        ?>

                        <!-- 제목 -->
                        <tr>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <?=$no?>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;text-align:center;">
								<?=date($_CFG['default_date_format'],$reg_time)?>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <?php
                                    // get current date
                                    $ts=time();
                                    // ending date
                                    $ending_ts=$row->ending_date; // time stamp
                                    $remaining_sec=($ending_ts-$ts);
//                                    echo $remaining_sec;
                                    $remaining_day=$remaining_sec/86400; // 86400 sec is 1 day
                                    $remaining_hours_sec=$remaining_sec%86400;

                                    $remaining_day=floor($remaining_day);
                                    if($remaining_day == 0){
                                    	echo floor($remaining_hours_sec/3600).' hours';
									}
									else {
                                    	echo $remaining_day.' days';
									}
                                ?>

                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?=$row->project_id?></a>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?=$row->task_name?></a>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <?=$row->idx?>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <?=$row->parent_task_idx?>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <div class="progress" style="margin: 0;">
                                    <div class="progress-bar progress-bar-<?php

                                    if ($row->task_progress >= 90 ) {
                                        echo 'success';
                                    } else if($row->task_progress >= 80){
                                        echo 'info';
                                    } else if($row->task_progress >= 50){
                                        echo 'warning';
                                    } else if($row->task_progress <= 49){
                                        echo 'danger';
                                    }

                                    ?>"
                                         role="progressbar" aria-valuenow="100"
                                         aria-valuemin="0" aria-valuemax="100" style="width:<?=$row->task_progress?>%">
                                        <?=$row->task_progress?> %
                                    </div>
                                </div>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?=$row->task_type?></a>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?=$row->task_priority?></a>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?=$row->bbs_writer?></a>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <a href="view?idx=<?=$row->idx?>"><?php
                                    // convert user id to user name
                                    $user_data =  get_user_data($row->assign_to);
                                    if($user_data){
//                                    print_r($user_data);
                                        echo $user_data->name;
                                    }
                                    else {
                                    	echo '';
									}

                                    ?></a>
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                            </td>
                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;">
                                <?=$row->sts?>
                            </td>

                            <td style="font-size:12px;background-color:#FFFFFF;padding:10px;font-weight:bold;text-align:left;">

                                <a href="write?idx=<?=$row->idx?>" title="edit" role="button" class="btn btn-primary btn-sm btn-dark"><em>edit</em></a>

                                <a href="set_deleted?idx=<?=$row->idx?>" onclick="if(!confirm('Are you sure?')){return false;}"
                                   title="delete" role="button" class="btn btn-primary btn-sm btn-dark"><em>delete</em></a>

                            </td>

                            <!--
                                <td>

                                    <?php if($is_admin){ ?>
                                <div class="buttons"
                                     style="text-align:right;background-color:white;white-space: nowrap;">
                                    <a onclick="location.href='<?=$path_arr['req_path2'];?>?idx=<?=$row->idx?>"
                                           title="수정하기" class="normal-btn small1" style=""><em>수정하기</em></a>
                                        <a onclick="if(confirm('정말 삭제하시겠습니까?')){location.href='<?=$path_arr['req_path2'];?>/delete?idx=<?=$row->idx?>&a=del_process&bbs_writer=<?=$row->bbs_name;?>&page=<?=$page?>&id=<?=$bbs_id;?>';}"
                                           class="normal-btn small1" style=""><em>삭제하기</em></a>
                                    </div>
                                    <?php } ?>

                                </td>
                                -->
                        </tr>
						<?php
						    $no = $no - 1;
						}
						?>

                    </table>


                    <style>
                        @media (max-width: 2000px) {
                            .divcont img {
                                display: inline-block;
                                width: auto \9 !important; /* ie8 */
                                width: auto !important;
                                max-width: 100%;
                                min-width: 100%;
                                height: auto !important;
                            }
                        }
                    </style>

                    <table border=0 cellspacing='0' cellpadding='7' width=100% height=25 style="padding-top:20px;">

                        <tr>
                            <td width=100>
                                <a href="list?page=<?=$page?>" role="button"  class="btn btn-primary btn-sm ladda-button" data-style="expand-right" data-size="l" style="float:;"><em>list</em></a>
                            </td>
                            <td colspan='3' align='center'>
                                <?php
                                //echo $pagelink;
                                //echo $pg->getPage();
                                ?>
                            </td>
                            <td align='right' width=100>
                                <? if($write_priv){ ?>
                                <a href="write" role="button" class="btn btn-primary btn-sm btn-dark" style="float:;"><em>post</em></a>
                                <? } ?>
                            </td>
                        </tr>
                    </table>

                <!-- 검색
                    <div id='search' align='center'>
                        <form id='search_form' name='search_form' method='GET' action=<?=$req_path2;?> onsubmit="return
                              form_chk();
                        ">
                        <input type='hidden' id='page' name='page' value='<?=$page?>'/>
                        <input type='hidden' id='id' name='id' value='<?=$bbs_id;?>'/>
                        <input type='hidden' id='search_mode' name='search_mode' value='1'/>
                        <input type='text' id='search_box' name='search_box' style="height:24px;"/>

                        <a onclick="document.getElementById('search_form').submit();" class="normal-btn small1"
                           style="float:;"><em>Search</em></a>
                        </form>
                    </div>
                    -->

                    <!-- 불펌방지 코드 시작 -->
                    <script type="text/javascript">
                        var omitformtags = ["input", "textarea", "select"]
                        omitformtags = omitformtags.join("|")

                        function disableselect(e) {
                            if (omitformtags.indexOf(e.target.tagName.toLowerCase()) == -1)
                                return false
                        }

                        function reEnable() {
                            return true
                        }

                        if (typeof document.onselectstart != "undefined")
                            document.onselectstart = new Function("return false")
                        else {
                            document.onmousedown = disableselect
                            document.onmouseup = reEnable
                        }
                    </script>
                    <!-- 불펌방지 코드 종료 -->

                </div>

            </div>
        </div>
    </section>
@endsection