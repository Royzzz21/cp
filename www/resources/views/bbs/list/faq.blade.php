    ...
    function faq(num){
        ...
        $('#faq_tr_'+num).show();
                    ...
                    foreach($list_info as $row){
                        ...
                        <table id='bbs_table' border=1 cellspacing='0' cellpadding='0' width=100%
                               style="padding:0 !important;;border:1px solid #d5d5d5;">

                            <!-- 제목 -->
                            <tr>
                                <td colspan=4
                                    style="font-size:13px;background-color:#FFFFFF;padding:10px;font-weight:bold;cursor:pointer;" onclick="faq(<?=$faq_idx?>);">
                                    <img src="/img/faq-q-icon.png" style="height:20px;padding-right:10px;"/><?=$row->bbs_subject?>

                                    <?php if($is_admin){ ?>
                                    <div class="buttons"
                                         style="text-align:right;background-color:white;white-space: nowrap;">
                                        <a onclick="location.href='write?idx=<?=$row->idx?>&mode=modify&a=write&id=<?=$bbs_id;?>&page=<?=$page?>'"
                                           title="수정하기" class="normal-btn small1" style=""><em>수정하기</em></a>
                                        <a onclick="if(confirm('정말 삭제하시겠습니까?')){location.href='delete?idx=<?=$row->idx?>&a=del_process&bbs_writer=<?=$row->bbs_name;?>&page=<?=$page?>&id=<?=$bbs_id;?>';}"
                            ...
                            <!-- 내용 -->
                            <tr id="faq_tr_<?=$faq_idx?>" style="display:none;">
                                <td colspan=4 valign='top'
                                    ...
                                    <img src="/img/faq-a-icon.png" style="height:20px;"/>
